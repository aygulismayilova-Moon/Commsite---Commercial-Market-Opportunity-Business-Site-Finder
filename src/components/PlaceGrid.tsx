...
import { Search, MapPin, Eye, Filter, ArrowUpDown, Plus, Trash2, Globe, Building2, Tag, Layers, AlertTriangle, X, Pencil, ChevronDown, Sparkles } from 'lucide-react';
import { LoadCityPlacesModal } from './LoadCityPlacesModal';
...
interface PlaceGridProps {
  ...
  onImportCityPlaces?: (places: PlaceItem[], replaceAll: boolean, cityName: string) => void;
}
...
export const PlaceGrid: React.FC<PlaceGridProps> = ({
  ...
  const [isLoadCityModalOpen, setIsLoadCityModalOpen] = useState(false);
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="p-3.5 bg-slate-50 border-b border-slate-200 space-y-2.5">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => setIsLoadCityModalOpen(true)}
              className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-bold bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 rounded shadow-xs transition-colors"
              title="Load authentic places and streets from Google Maps for any chosen city"
            >
              <Globe className="w-3.5 h-3.5 text-indigo-600" />
              <span>Fetch City Places</span>
            </button>
            <button
              onClick={onAddPlace}
      ...
      {/* Load Real City Places Modal */}
      {isLoadCityModalOpen && (
        <LoadCityPlacesModal
          isOpen={isLoadCityModalOpen}
          onClose={() => setIsLoadCityModalOpen(false)}
          onImportPlaces={(importedPlaces, replaceAll, cityName) => {
            if (onImportCityPlaces) {
              onImportCityPlaces(importedPlaces, replaceAll, cityName);
            }
          }}
          currentCityName={places[0]?.city || 'London'}
          currentCountryName={places[0]?.country || 'United Kingdom'}
        />
      )}
    </div>
  );
