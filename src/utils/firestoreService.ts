...
function isOfflineOrUnavailable(error: any): boolean {
  if (!error) return false;
  const str = String(error?.message || error?.code || error || '');
  return (
    str.includes('unavailable') ||
    str.includes('offline') ||
    str.includes('Could not reach Cloud Firestore') ||
...
function handleFirestoreError(
  ...
  if (isOfflineOrUnavailable(error)) {
    console.warn(`[Firestore Status] ${actionName}: Firestore is running in offline cached mode.`);
    return;
  }
