...
export interface GenerateMapCanvasOptions {
  placeName: string;
  area?: string;
  city?: string;
  category?: string;
...
export async function generateMapCanvasDataUrl(opts: GenerateMapCanvasOptions): Promise<string> {
  const result = await captureGoogleMapSnapshot({
    placeName: opts.placeName,
    lat: opts.latitude,
    lng: opts.longitude,
    zoom: opts.zoom,
    mapType: opts.mapType || 'satellite',
    dateText: opts.dateText || new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
    eventType: opts.eventType || 'Baseline',
  });
  return result;
}
