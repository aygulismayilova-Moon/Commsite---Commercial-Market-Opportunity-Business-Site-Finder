...
import { CommercialMarketFinder } from './components/CommercialMarketFinder';
...
export default function App() {
  ...
  // Primary View Mode: 'commercial' | 'monitoring' | 'admin' | 'login'
  const [activeView, setActiveView] = useState<'commercial' | 'monitoring' | 'admin' | 'login'>('commercial');
  ...
  // Import city places from Google Maps
  const handleImportCityPlaces = (newPlaces: PlaceItem[], replaceAll: boolean, _cityName: string) => {
    if (replaceAll) {
      ...
      const existingNames = new Set(places.map((p) => p.place_name.trim().toLowerCase()));
      const toAdd = newPlaces.filter((p) => !existingNames.has(p.place_name.trim().toLowerCase()));
      const combined = [...places, ...toAdd];
      updatePlaces(combined);
      if (toAdd.length > 0) {
        setSelectedPlaceId(toAdd[0].id);
      }
    }
  };
      ...
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
        <div className="text-center space-y-4 max-w-sm">
          ...
          <div className="flex items-center justify-center gap-2 text-blue-400 text-xs font-semibold">
            ...
            <span>Verifying system clearance &amp; credentials...</span>
          </div>
        </div>
      </div>
    );
  ...
  // 2. Strict Authentication Guard: If user is not logged in, show ONLY the login page
  if (!userProfile) {
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between selection:bg-blue-600 selection:text-white">
        <div className="flex-1 flex items-center justify-center p-4">
          <LoginPage
            onSuccess={() => setActiveView('commercial')}
          />
        </div>
        <footer className="py-3 text-center text-xs text-slate-500 border-t border-slate-900 bg-slate-950/80">
          <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-2">
            <span className="font-semibold text-slate-400">
              COMMSITE &copy; {new Date().getFullYear()} - Commercial Market Opportunity &amp; Business Site Finder
            </span>
            <span className="text-[11px] text-slate-600 font-mono">
              Secure Access Gateway &bull; Role-Based Access Control
            </span>
          </div>
        </footer>
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans antialiased">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-5 space-y-6">
        {activeView === 'admin' ? (
          ...
          <AdminSection
            ...
            onNavigateToCommercial={() => setActiveView('commercial')}
          />
          ...
          <>
                  ...
                  <div className="pt-1 border-t border-slate-100 space-y-1.5">
                    <div className="flex items-center gap-2">
                      <span className="px-2.5 py-0.5 text-xs font-bold bg-indigo-50 text-indigo-800 rounded-full border border-indigo-200 flex items-center gap-1">
                        ...
                        Head Office And Branches - Physical Monitoring
                      </span>
                    </div>
            ...
            <section ref={placeGridRef} className="scroll-mt-20">
              <PlaceGrid
                ...
                onImportCityPlaces={handleImportCityPlaces}
              />
            </section>
      ...
      <footer className="bg-white border-t border-slate-200 py-3 mt-8 text-center text-xs text-slate-500">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          <span className="font-bold text-slate-700">COMMSITE &copy; {new Date().getFullYear()} - Commercial Market Intelligence &amp; Geospatial Control</span>
